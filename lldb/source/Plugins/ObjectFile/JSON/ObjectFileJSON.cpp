...
ObjectFileJSON::ObjectFileJSON(const ModuleSP &module_sp, DataBufferSP &data_sp,
                               ...
                               UUID uuid, std::vector<Symbol> symbols)
...
bool fromJSON(const json::Value &value, ObjectFileJSON::Symbol &symbol,
              json::Path path) {
  json::ObjectMapper o(value, path);
  return o && o.map("addr", symbol.addr) && o.map("name", symbol.name);
}

bool fromJSON(const json::Value &value, ObjectFileJSON::Symbols &symbols,
              json::Path path) {
  json::ObjectMapper o(value, path);
  return o && o.map("symbols", symbols.symbols);
}
