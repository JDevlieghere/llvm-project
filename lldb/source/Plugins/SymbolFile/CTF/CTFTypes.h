...
#ifndef LLDB_SOURCE_PLUGINS_SYMBOLFILE_CTF_CTFTYPE_H
#define LLDB_SOURCE_PLUGINS_SYMBOLFILE_CTF_CTFTYPE_H
...
public:
  ...
  CTFRecord(Kind kind, lldb::user_id_t uid, llvm::StringRef name,
      ...
      : CTFType(kind, uid, name), nfields(nfields), size(size), fields(fields) {
    assert(fields.size() == nfields);
  }
...
struct CTFStruct : public CTFRecord {
  CTFStruct(lldb::user_id_t uid, llvm::StringRef name, uint32_t nfields,
      ...
      : CTFRecord(eStruct, uid, name, nfields, size, fields){};
};

struct CTFUnion : public CTFRecord {
  CTFUnion(lldb::user_id_t uid, llvm::StringRef name, uint32_t nfields,
           uint32_t size, std::vector<Field> fields)
      : CTFRecord(eUnion, uid, name, nfields, size, fields){};
};

struct CTFForward : public CTFType {
  CTFForward(lldb::user_id_t uid, llvm::StringRef name, uint32_t forward_kind)
      : CTFType(eForward, uid, name), forward_kind(forward_kind) {}

  uint32_t forward_kind;
};

} // namespace lldb_private

#endif // LLDB_SOURCE_PLUGINS_SYMBOLFILE_CTF_CTFTYPE_Hu
