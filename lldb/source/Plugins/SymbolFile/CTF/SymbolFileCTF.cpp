...
bool SymbolFileCTF::ParseHeader() {
  ...
  offset += m_data.ExtractBytes(offset, ctf_header_size, eByteOrderLittle,
                                &ctf_header);

  // Validate the preamble.
  if (ctf_header.preamble.magic != 0xcff1) {
    LLDB_LOG(log, "CTF parsing failed: invalid magic: {0:x}",
             ctf_header.preamble.magic);
    return false;
  }

  if (ctf_header.preamble.version != 4) {
  ...
  if (!m_data.ValidOffset(header_offset + ctf_header.stroff +
                          ctf_header.parlabel)) {
    LLDB_LOG(log, "CTF parsing failed: invalid parent label offset: {0}",
             ctf_header.parlabel);
    return false;
  }
  ...
  if (!m_data.ValidOffset(header_offset + ctf_header.stroff +
                          ctf_header.parname)) {
    LLDB_LOG(log, "CTF parsing failed: invalid parent name offset: {0}",
             ctf_header.parname);
    return false;
  }
...
static uint32_t GetEncoding(uint32_t data) { return ((data)&0xff000000) >> 24; }

static uint32_t GetBits(uint32_t data) { return (data)&0x0000ffff; }

static uint32_t GetBytes(uint32_t bits) { return bits / sizeof(unsigned); }

uint32_t GetKind(uint32_t data) { return ((data)&0xf800) >> 11; }

uint32_t GetVLen(uint32_t data) { return (data)&0x3ff; }
...
llvm::Expected<TypeSP> SymbolFileCTF::ParseInteger(lldb::offset_t &offset,
  ...
  if (basic_type != eBasicTypeVoid) {
    ...
    if (!compiler_type.IsIntegerType(compiler_type_is_signed))
      return llvm::make_error<llvm::StringError>(
          llvm::formatv("found compiler type for '{0}' but not an integer type",
                        name),
          llvm::inconvertibleErrorCode());
...
llvm::Expected<lldb::TypeSP> SymbolFileCTF::ParseArray(lldb::offset_t &offset,
  ...
  offset += m_data.ExtractBytes(offset, sizeof(ctf_array_t), eByteOrderLittle,
                                &ctf_array);
...
llvm::Expected<lldb::TypeSP> SymbolFileCTF::ParseEnum(lldb::offset_t &offset,
  ...
  for (uint32_t i = 0; i < elements; ++i) {
    ctf_enum_t ctf_enum;
    offset += m_data.ExtractBytes(offset, sizeof(ctf_enum_t), eByteOrderLittle,
                                  &ctf_enum);
...
SymbolFileCTF::ParseRecord(lldb::offset_t &offset, lldb::user_id_t uid,
  ...
  for (uint32_t i = 0; i < fields; ++i) {
    ...
    offset += m_data.ExtractBytes(offset, sizeof(ctf_member_t),
                                  eByteOrderLittle, &ctf_member);
...
size_t SymbolFileCTF::ParseTypes(CompileUnit &cu) {
  ...
  lldb::user_id_t type_uid = 0;
  while (type_offset < type_offset_end) {
    ctf_stype_t ctf_stype;
    m_data.ExtractBytes(type_offset, sizeof(ctf_stype_t), eByteOrderLittle,
                        &ctf_stype);
    ...
    lldb::offset_t next_type_offset = type_offset + ctf_stype.GetStructSize();
    type_uid++;

    TypeSP type_sp;
    llvm::Expected<TypeSP> type_or_error = ParseType(
        next_type_offset, type_uid, name, kind, variable_length, type, size);
    if (!type_or_error) {
...
void SymbolFileCTF::GetTypes(SymbolContextScope *sc_scope, TypeClass type_mask,
